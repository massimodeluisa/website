...
const NAME_REVEAL_DURATION = 0.82
const NAME_REVEAL_STAGGER = 0.011
const NAME_REVEAL_DELAY = 0.08
const KICKER_REVEAL_DURATION = 0.45
const KICKER_REVEAL_STAGGER = 0.018
const TAGLINE_FADE_DURATION = 0.4
...
const createTextReveal = () => {
  if (nameEl.value) {
    revealChars(nameEl.value, {
      scrollTrigger: false,
      duration: NAME_REVEAL_DURATION,
      stagger: NAME_REVEAL_STAGGER,
      delay: NAME_REVEAL_DELAY,
    })
  }
...
const runTaglineCycle = async (): Promise<void> => {
  ...
  if (parent) {
    gsap.to(parent, { opacity: 1, duration: TAGLINE_FADE_DURATION, ease: 'power2.out' })
  }
  ...
  while (!cancelled) {
    ...
    // Back to a single plain text node so TextPlugin can own it again.
    split.revert()
    activeSplit = null

    // Phase 3 — typewriter erase (scomparsa): un-type the line before the swap.
    const eraseTl = gsap.timeline()
    activeTl = eraseTl
    eraseTl.to(el, {
      text: { value: '' },
      duration: Math.max(0.3, targetText.length / 42),
      ease: 'none',
    })
    await playTimeline(eraseTl)
...
.min-w-0.text-center(class="md:text-left")
  ...
  h1.select-none.font-semibold.text-site-heading.opacity-0(
    ...
    class="text-[clamp(2.4rem,5vw,4.4rem)] leading-[0.76] tracking-[-0.035em]"
  )
...
/* Left-align tagline lines from `md` up, matching the parent's `md:text-left`
   (kicker/name flip to left at 768px; the line block must follow). */
